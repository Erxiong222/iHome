...
func GetUserHouse(userName string)([]*house.Houses,error){
	...
	for _,v := range houses{
		...
		houseInfo.ImgUrl ="http://127.0.0.1:8888/"+ v.Index_image_url
		houseInfo.OrderCount = int32(v.Order_count)
		houseInfo.Price = int32(v.Price)
		houseInfo.RoomCount = int32(v.Room_count)
		houseInfo.UserAvatar = "http://127.0.0.1:8888/"+user.Avatar_url
...
func GetHouseDetail(houseId,userName string)(house.DetailData,error){
	...
	}
	{
		...
		if houseInfo.Index_image_url != ""{
			houseDetail.ImgUrls = append(houseDetail.ImgUrls,"http://127.0.0.1:8888/"+houseInfo.Index_image_url)
		}
	}
	...
	for _,v := range imgs{
		if len(imgs) != 0 {
			houseDetail.ImgUrls = append(houseDetail.ImgUrls,"http://127.0.0.1:8888/"+v.Url)
		}
	}
	...
	houseDetail.UserAvatar = "http://127.0.0.1:8888/"+user.Avatar_url
...
func GetIndexHouse()([]*house.Houses,error){
	...
	for _, v := range houses{
		...
		houseTemp.ImgUrl = "http://127.0.0.1:8888/"+v.Index_image_url
		...
		houseTemp.UserAvatar = "http://127.0.0.1:8888/"+user.Avatar_url
